...
class PostgreSQLAbstractCacheHandler(AbstractCacheHandler_Lazy):
    ...
    def set_null(self, key: str, partition_key: str = "partition_key") -> bool:
        ...
        try:
            ...
            self.cursor.execute(
                sql.SQL("INSERT INTO {0} (query_hash, value) VALUES (%s, %s) "
                        "ON CONFLICT (query_hash) DO UPDATE SET value = EXCLUDED.value").format(
    ...
    def exists(self, key: str, partition_key: str = "partition_key") -> bool:
        ...
        try:
            ...
            table_name = f"{self.tableprefix}_cache_{partition_key}"
            self.cursor.execute(
                sql.SQL("SELECT 1 FROM {0} WHERE query_hash = %s").format(
                    sql.Identifier(table_name)
                ),
                (key,)
            )
    ...
    def delete(self, key: str, partition_key: str = "partition_key") -> bool:
        ...
        try:
            ...
            table_name = f"{self.tableprefix}_cache_{partition_key}"
            self.cursor.execute(
                sql.SQL("DELETE FROM {0} WHERE query_hash = %s").format(
                    sql.Identifier(table_name)
                ),
                (key,)
            )
            ...
            self.cursor.execute(
                sql.SQL("DELETE FROM {0} WHERE partition_key = %s AND query_hash = %s").format(
    ...
    def delete_partition(self, partition_key: str) -> bool:
        ...
        try:
            ...
            self.cursor.execute(
                sql.SQL("DELETE FROM {0} WHERE partition_key = %s").format(
                    sql.Identifier(self.tableprefix + "_queries")
                ),
                (partition_key,)
            )
    ...
    def prune_old_queries(self, days_old: int = 30) -> int:
        ...
        try:
            ...
            # Delete from queries table
            self.cursor.execute(
                sql.SQL("DELETE FROM {0} WHERE last_seen < now() - interval '%s days'").format(
                    sql.Identifier(self.tableprefix + "_queries")
                ),
                (days_old,)
            )
    ...
    def get_partition_keys(self) -> list[tuple[str, str]]:
        ...
        try:
            self.cursor.execute(
                sql.SQL("SELECT partition_key, datatype FROM {} ORDER BY partition_key").format(
                    sql.Identifier(self.tableprefix + "_partition_metadata")
                )
            )

...
from partitioncache.query_processor import generate_all_query_hash_pairs, clean_query, hash_query
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="SQL query to add to cache"
    )
    
    parser.add_argument(
        "--queue",
        action="store_true",
        help="Add query to queue instead of executing directly"
    )
    
    parser.add_argument(
        "--no-recompose",
        action="store_true",
        help="Do not recompose the query before adding to cache, if true the query is added as is"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="postgresql",
        help="Database backend (currently only postgresql supported)"
    )
    ...
    parser.add_argument(
        "--env-file",
        type=str,
        help="Path to environment file with database credentials"
    )

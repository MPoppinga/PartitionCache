...
def test_set_set(cache_handler):
    ...
    cache_handler.cursor.execute.assert_called_with("INSERT INTO test_cache_table_cache VALUES (%s, %s)", ("key1", [1, 2, 3]))
...
def test_get(cache_handler):
    ...
    assert result == {1, 2, 3}
    cache_handler.cursor.execute.assert_called_with("SELECT value FROM test_cache_table_cache WHERE query_hash = %s", ("key1",))
...
def test_set_null(cache_handler):
    ...
    cache_handler.cursor.execute.assert_called_with("INSERT INTO test_cache_table_cache VALUES (%s, %s)", ("null_key", None))
...
def test_delete(cache_handler):
    ...
    cache_handler.cursor.execute.assert_called_with("DELETE FROM test_cache_table_cache WHERE query_hash = %s", ("key_to_delete",))
    cache_handler.db.commit.assert_called()
...
def test_set_set_large_numbers(cache_handler):
    ...
    cache_handler.cursor.execute.assert_called_with("INSERT INTO test_cache_table_cache VALUES (%s, %s)", ("large_key", [1000000, 2000000, 3000000]))
...
def test_delete_non_existent_key(cache_handler):
    ...
    cache_handler.cursor.execute.assert_called_with("DELETE FROM test_cache_table_cache WHERE query_hash = %s", ("non_existent_key",))
    cache_handler.db.commit.assert_called()


def test_set_set_update_existing(cache_handler):
    cache_handler.set_set("existing_key", {1, 2, 3})
    cache_handler.cursor.execute.assert_called_with("INSERT INTO test_cache_table_cache VALUES (%s, %s)", ("existing_key", [1, 2, 3]))

    # Update the existing key
    cache_handler.set_set("existing_key", {4, 5, 6})
    cache_handler.cursor.execute.assert_called_with("INSERT INTO test_cache_table_cache VALUES (%s, %s)", ("existing_key", [4, 5, 6]))
...
def test_very_long_key(cache_handler):
    ...
    cache_handler.cursor.execute.assert_called_with("INSERT INTO test_cache_table_cache VALUES (%s, %s)", (long_key, [1, 2, 3]))
...
def test_unicode_key(cache_handler):
    ...
    cache_handler.cursor.execute.assert_called_with("INSERT INTO test_cache_table_cache VALUES (%s, %s)", (unicode_key, [1, 2, 3]))

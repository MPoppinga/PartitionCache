...
class TestPgCronIntegration:
    ...
    @pytest.mark.slow
    def test_full_pg_cron_workflow(self, db_session, cache_client):
        """
        ...
        with db_session.cursor() as cur:
            try:
                cur.execute(
                    "DELETE FROM cron.job WHERE jobname LIKE %s",
                    (f"{db_name}_pcache_test_%",)
                )
                db_session.commit()
        ...
        queue_prefix = os.getenv("PG_QUEUE_TABLE_PREFIX", "partitioncache_queue")
        
        try:
            # Initialize processor tables if not exists
            with db_session.cursor() as cur:
                cur.execute(
                    "SELECT partitioncache_initialize_processor_tables(%s)",
                    (queue_prefix,)
                )
                db_session.commit()
            ...
            with db_session.cursor() as cur:
                cur.execute(
                    "SELECT jobname, schedule, active FROM cron.job WHERE jobname LIKE %s",
                    (f"{test_job_name}_%",)
                )
                jobs = cur.fetchall()
            ...
            assert final_length < initial_queue_length, \
                f"pg_cron did not process queries. Initial: {initial_queue_length}, Final: {final_length}"
            ...
            with db_session.cursor() as cur:
                try:
                    ...
                    cur.execute(
                        "UPDATE %s SET enabled = false WHERE job_name = %%s" % config_table,
                        (test_job_name,)
                    )
                    
                    # This should trigger removal of cron jobs
                    cur.execute(
                        "DELETE FROM %s WHERE job_name = %%s" % config_table,
                        (test_job_name,)
                    )
    ...
    def test_pg_cron_job_isolation(self, db_session):
        """
        ...
        try:
            ...
            with db_session.cursor() as cur:
                for i, prefix in enumerate([job_prefix_1, job_prefix_2]):
                    ...
                    cur.execute(
                        "SELECT cron.schedule(%s, '* * * * *', 'SELECT 1;')",
                        (job_name,)
                    )
            ...
            with db_session.cursor() as cur:
                cur.execute(
                    "SELECT jobname FROM cron.job WHERE jobname IN (%s, %s)",
                    (f"{job_prefix_1}_job", f"{job_prefix_2}_job")
                )
                jobs = cur.fetchall()

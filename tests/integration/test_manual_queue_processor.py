...
class TestManualQueueProcessor:
    ...
    def test_manual_queue_processing(self, db_session, cache_client, postgresql_queue_processor):
        ...
        assert final_lengths["original_query_queue"] < lengths["original_query_queue"], "Queue should be reduced after manual processing"

    def test_manual_processing_with_cache_population(self, db_session, cache_client, postgresql_queue_processor):
        ...
        success = push_to_original_query_queue(query=test_query, partition_key=partition_key, partition_datatype="integer")
    ...
    def test_manual_processing_empty_queue(self, db_session, postgresql_queue_processor):
    ...
    def test_manual_processing_partial_batch(self, db_session, postgresql_queue_processor):
        ...
        for query in test_queries:
            push_to_original_query_queue(query=query, partition_key=partition_key, partition_datatype="integer")
    ...
    def test_manual_processing_different_partitions(self, db_session, postgresql_queue_processor):
        ...
        for partition_key, queries in queries_by_partition.items():
            ...
            for query in queries:
                push_to_original_query_queue(query=query, partition_key=partition_key, partition_datatype=datatype)
                total_queries += 1
    ...
    @pytest.mark.slow
    def test_manual_processing_performance(self, db_session, postgresql_queue_processor):
...
class TestManualProcessingIntegration:
    ...
    def test_manual_processing_with_postgresql_queue_processor_cli(self, db_session, postgresql_queue_processor):
        ...
        # Add a test query
        push_to_original_query_queue(query="SELECT * FROM test_locations WHERE zipcode = 6001;", partition_key="zipcode", partition_datatype="integer")
    ...
    def test_manual_processing_respects_queue_order(self, db_session, postgresql_queue_processor):

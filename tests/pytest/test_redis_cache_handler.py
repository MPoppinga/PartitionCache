...
def test_get_non_existent_key(cache_handler, mock_redis):
    ...
    # First call to type() checks metadata key, returns string indicating metadata exists
    # Second call to type() checks cache key, returns none indicating key doesn't exist
    mock_redis.type.side_effect = [b"string", b"none"]

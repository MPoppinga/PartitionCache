...
def test_normalize_distance_conditions_between():
    ...
    assert (
        normalize_distance_conditions(query, restrict_to_dist_functions=False)
        == expected_result
    )
    ...
    assert (
        normalize_distance_conditions(query, restrict_to_dist_functions=True)
        == expected_result
    )
    ...
    expected_result = (
        "SELECT * FROM table AS a, table AS b WHERE DIST(a, b) BETWEEN 1 AND 4"
    )
    assert (
        normalize_distance_conditions(query, restrict_to_dist_functions=False)
        == expected_result
    ...
    assert (
        normalize_distance_conditions(
            query, bucket_steps=0.5, restrict_to_dist_functions=False
        )
        == expected_result
    )
...
def test_normalize_distance_conditions():
    ...
    expected_result = (
        "SELECT * FROM table_distance AS td WHERE td.distance > 1 AND td.distance < 4"
    )
    assert (
        normalize_distance_conditions(query, restrict_to_dist_functions=False)
        == expected_result
    ...
    query = "SELECT * FROM table_distance AS td WHERE td.distance > 1.6 AND td.distance > 3.6"
    expected_result = (
        "SELECT * FROM table_distance AS td WHERE td.distance > 1 AND td.distance > 3"
    )
    assert (
        normalize_distance_conditions(query, restrict_to_dist_functions=False)
        == expected_result
    ...
    query = "SELECT * FROM table_distance AS td WHERE td.distance < 1.6 AND td.distance < 3.6"
    expected_result = (
        "SELECT * FROM table_distance AS td WHERE td.distance < 2 AND td.distance < 4"
    )
    assert (
        normalize_distance_conditions(query, restrict_to_dist_functions=False)
        == expected_result
    ...
    expected_result = "SELECT * FROM table_distance AS td, td2 WHERE SQRT((td.x - td2.x) ^ 2 + (td.y - td2.y) ^ 2) > 1 AND SQRT((td.x - td2.x) ^ 2 + (td.y - td2.y) ^ 2) < 4"
...
def test_subquery_builder_2_tables():
    ...
    assert sorted(
        generate_partial_queries(
            query, partition_key, min_component_size, follow_graph, fix_attributes
        )

#!/usr/bin/env python3
"""
SSB Benchmark Evaluation Script for PartitionCache.

Demonstrates PartitionCache's multi-partition-key support and cross-dimension
cache reuse using the Star Schema Benchmark (SSB).

Each lineorder FK column is a separate partition key:
  lo_custkey, lo_suppkey, lo_partkey, lo_orderdate

For each adapted SSB query, we generate variants per relevant partition key.
Shared conditions across queries produce identical hashes -> cache hits.
...
Usage:
    ...
    python run_ssb_benchmark.py --scale-factor 0.01 --db-backend duckdb --mode flight 3
...
QUERY_PARTITION_KEYS = {
    # Q1 flight: only date + lineorder attrs
    "q1_1": ["lo_orderdate"],
    "q1_2": ["lo_orderdate"],
    "q1_3": ["lo_orderdate"],
    # Q2 flight: supplier + part
    "q2_1": ["lo_suppkey", "lo_partkey"],
    "q2_2": ["lo_suppkey", "lo_partkey"],
    "q2_3": ["lo_suppkey", "lo_partkey"],
    # Q3 flight: customer + supplier + date
    "q3_1": ["lo_custkey", "lo_suppkey", "lo_orderdate"],
    "q3_2": ["lo_custkey", "lo_suppkey", "lo_orderdate"],
    "q3_3": ["lo_custkey", "lo_suppkey", "lo_orderdate"],
    "q3_4": ["lo_custkey", "lo_suppkey", "lo_orderdate"],
    # Q4 flight: all 4 dimensions
    "q4_1": ["lo_custkey", "lo_suppkey", "lo_partkey", "lo_orderdate"],
    "q4_2": ["lo_custkey", "lo_suppkey", "lo_partkey", "lo_orderdate"],
    "q4_3": ["lo_custkey", "lo_suppkey", "lo_partkey", "lo_orderdate"],
    # Q5: customer hierarchy drill-down (region -> nation -> city)
    "q5_1": ["lo_custkey", "lo_suppkey", "lo_orderdate"],
    "q5_2": ["lo_custkey", "lo_suppkey", "lo_orderdate"],
    "q5_3": ["lo_custkey", "lo_suppkey", "lo_orderdate"],
    # Q6: part hierarchy drill-down (mfgr -> category -> brand)
    "q6_1": ["lo_partkey", "lo_suppkey", "lo_orderdate"],
    "q6_2": ["lo_partkey", "lo_suppkey", "lo_orderdate"],
    "q6_3": ["lo_partkey", "lo_suppkey", "lo_orderdate"],
    # Q7: single partition key
    "q7_1": ["lo_custkey"],
    "q7_2": ["lo_suppkey"],
    "q7_3": ["lo_partkey"],
    # Q8: multi-partition key combinations
    "q8_1": ["lo_custkey", "lo_orderdate"],
    "q8_2": ["lo_custkey", "lo_partkey"],
    "q8_3": ["lo_suppkey", "lo_orderdate"],
}
...
class PostgreSQLExecutor:
    ...
    def __init__(self):
        ...
        self.conn = psycopg.connect(
            ...
            dbname=os.getenv("DB_NAME", "ssb_db"),
        )
...
class CacheManager:
    ...
    def __init__(self, cache_type, db_path=None):
    ...
    def get_handler(self, partition_key):
        ...
        if partition_key not in self.handlers:
            if self.cache_type == "duckdb_bit":
                cache_db = str(self.db_path).replace(".duckdb", "_cache.duckdb") if self.db_path else "ssb_cache.duckdb"
                # Set environment for DuckDB bit cache
                os.environ["DUCKDB_BIT_PATH"] = cache_db
                # Date keys are YYYYMMDD integers (up to ~19981231), so bitsize must be > 20M
                os.environ["DUCKDB_BIT_BITSIZE"] = "20000000"
...
def run_flight(executor, cache_manager, query_dir, flight_num, repeat=1):
    ...
    print(f"\n{'=' * 80}")
    print(f"SSB Benchmark: Flight Q{flight_num}")
    print(f"{'=' * 80}")
...
def run_cross_dimension(executor, cache_manager, query_dir, repeat=1):
    ...
    Demonstrates how cache entries from one query benefit subsequent queries
    through shared condition variants.
    """
    print("\n" + "=" * 80)
    print("SSB Benchmark: Cross-Dimension Cache Reuse")
    print("=" * 80)

    results = []

    # Phase 1: Run Q3 flight sequentially, showing cumulative cache reuse
    print("\n### Phase 1: Q3 Flight (customer + supplier + date)")
    print("Demonstrates cache reuse across queries with shared dimension filters.\n")
    ...
    print("\n### Phase 2: Q4 Flight (all 4 dimensions)")
    print("Q4 queries share customer+supplier+date conditions with Q3 -> cache reuse.\n")
...
def run_hierarchy(executor, cache_manager, query_dir, repeat=1):
    ...
    Runs Q5 (customer hierarchy: region -> nation -> city) then Q6 (part hierarchy:
    mfgr -> category -> brand) sequentially. Demonstrates increasing cache reuse
    as queries drill from broad to narrow within the same dimension.
    """
    print("\n" + "=" * 80)
    print("SSB Benchmark: Hierarchical Drill-Down")
    print("=" * 80)

    all_results = []

    for flight_num, flight_label in [(5, "Q5: Customer Hierarchy (region -> nation -> city)"),
                                      (6, "Q6: Part Hierarchy (mfgr -> category -> brand)")]:
...
def run_backend_comparison(executor, cache_manager, query_dir, db_backend, db_path=None, repeat=1):
    ...
    """
    print("\n" + "=" * 80)
    print("SSB Benchmark: Backend Comparison")
    print("=" * 80)
    ...
    for backend in backends:
        ...
        try:
            cm = CacheManager(backend, db_path=db_path)
            # Test that we can create a handler
            cm.get_handler("lo_orderdate")
        except Exception as e:
            print(f"  Skipped: {e}")
            continue
...
def main():
    parser = argparse.ArgumentParser(description="Run SSB benchmark with PartitionCache")
    ...
    if args.db_backend == "duckdb":
        ...
        if not os.path.exists(db_path):  # type: ignore[arg-type]
            ...
            print(f"Generate data first: python generate_ssb_data.py --scale-factor {args.scale_factor} --db-backend duckdb")
            sys.exit(1)
